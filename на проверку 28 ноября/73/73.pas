PROGRAM BubbleSortDown(INPUT, OUTPUT);  { СортируемпервуюстрокуINPUTвOUTPUT }
VAR
      ...
      THEN
        BEGIN
          ...
          DO{ По крайней мере два символа остается для Ch1,Ch2 }
            BEGIN
              READ(F1, Ch2);
              IF Ch1 >= Ch2  { Выводим   max(Ch1,Ch2) в  F2, записывая отсортированные символы }
              THEN
                BEGIN
                  WRITE(F2, Ch1);
                  Ch1 := Ch2
        ...
        END
    END; {КопируемF1 вF2,проверяя отсортированность и переставляя первые соседнии символы по порядку}
