...
BEGIN {SelectSort}
  ...
  BEGIN {Сортировать F1 в OUTPUT используя стратегию SelectSort}
    ...
    DO { Ch<> '#' и Ch1 - первый символ F1}
            ...
            BEGIN
              BEGIN {Выбираем минимальный из (Ch, Min) копируем второй символ в F2}
                ...
                ELSE {Min - минимальный из (Ch, Min)}
                  WRITE(F2, Ch);
              END; {Выбираем минимальный из (Ch, Min) копируем второй символ в F2}
              READ(F1, Ch)
            END;
          WRITELN(F2, '#');
        END; {Выбираем минимальный из F1 b копируем остаток F1 в F2}
        WRITE(OUTPUT, Min);
      ...
      END;  
    WRITELN(OUTPUT);
  END {Сортировать F1 в OUTPUT, используя стратегию SelectSort}
END. {SelectSort}
